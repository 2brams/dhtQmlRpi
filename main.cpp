...
QObject *itemT = nullptr;

QObject *itemH = nullptr;
double value = 10;
...
int read_dht22_dat()
{
    ...
    if ((j >= 40) &&
        ...
        if(itemT)
            itemT->setProperty("text",QString().setNum(t,'f',1));
        if(itemH)
...
int update()
{
    ...
    value++;
    if(itemT)
        itemT->setProperty("text",QString().setNum(value,'f',1));
...
int main(int argc, char *argv[])
{
    ...
    itemT = engine.rootObjects().at(0)->findChild<QQuickItem*>("temp");
